...
class StripeWebhook
{
	...
	public function __construct()
	{
		...
		$user_exists = $this->users->find($user_data['email']);

		if (empty($user_exists)) { // New Customer
			$this->user = $this->users->create($user_data['email']);
		} else { // Customer exists
			$this->user = $user_exists;
		}

		if (empty($this->user['stripe_customer_id']))
			$this->users->setField('users', $this->user['usersId'], 'stripe_customer_id', $user_data['stripe_customer_id']);
