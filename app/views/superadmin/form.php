...
<div class="mx-auto max-w-5xl  mt-10 min-h-screen">
 <div class="mx-auto text-center mb-10">
   
     
        <h2 class="text-3xl"><? if ($rid != -1) { echo ucfirst($table_label).' #'.$rid; ?>
	<!-- Meta data -->
	<small class="text-xs text-gray-400 block font-italic"> Creado <?= strftime(" %d %B %Y %H:%M",strtotime($raw['created'])) ?><br> Actualizado <?= strftime(" %d %B %Y %H:%M",strtotime($raw['updated'])) ?></small>
       <?} else echo ucfirst($table).' > Añadir nuevo '; ?> </h2>
...
<div class="<? if ($rid != -1): echo 'xlg:grid xlg:grid-cols-10 xgap-4'; endif; ?> ">
